 ...
 */
//@formatter:on
public class WindowDragAndDrop {
    ...
    protected WindowDragAndDrop(MaterialWidget target) {
        this.target = target;
    }

    protected WindowDragAndDrop draggable() {
        if (jsDnd == null) {
    ...
    public WindowDragAndDrop draggable(JsDragOptions options) {
        dragOptions = options;
    ...
    public static WindowDragAndDrop draggable(MaterialWidget target) {
        return draggable(target, JsDragOptions.create());
    }

    public static WindowDragAndDrop draggable(MaterialWidget target, JsDragOptions options) {
        return new WindowDragAndDrop(target).draggable(options);
    }

    protected WindowDragAndDrop dropzone() {
        if (jsDnd == null) {
            jsDnd = JsDnd.interact(target.getElement());
    ...
    public WindowDragAndDrop dropzone(JsDropOptions options) {
        dropOptions = options;
        if (target.isAttached()) {
            dropzone();
        } else {
    ...
    public static WindowDragAndDrop dropzone(MaterialWidget target) {
        return dropzone(target, JsDropOptions.create());
    }

    public static WindowDragAndDrop dropzone(MaterialWidget target, JsDropOptions options) {
        return new WindowDragAndDrop(target).dropzone(options);
    }
