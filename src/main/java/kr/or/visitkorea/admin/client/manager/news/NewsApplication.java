package kr.or.visitkorea.admin.client.manager.news;

import java.util.Map;

import kr.or.visitkorea.admin.client.application.ApplicationView;
import kr.or.visitkorea.admin.client.manager.ApplicationBase;
import kr.or.visitkorea.admin.client.manager.news.composite.NewsMain;
import kr.or.visitkorea.admin.client.manager.news.dialogs.SearchContentsDialog;
import kr.or.visitkorea.admin.client.widgets.window.MaterialExtentsWindow;

public class NewsApplication extends ApplicationBase{
	
	public static final String SEARCH_TITLE = "SEARCH_TITLE";

	
	public NewsApplication(ApplicationView applicationView) {
		super(applicationView);
	}

	public void setWindow(MaterialExtentsWindow materialExtentsWindow, String divisionName) {
		this.setDivisionName(divisionName);
		this.windowLiveFlag = true;
		this.window = materialExtentsWindow;
		this.window.addDialog(SEARCH_TITLE, new SearchContentsDialog(this.window)); 
		this.window.addCloseHandler(event->{
			windowLiveFlag = false;
		});
	}
	
	public void start() {

		start(null);
		
	}
	@Override
	public void start(Map<String, Object> params) {
		
		this.params = params;
		this.window.add(new NewsMain(this.window));
		this.window.open(this.window);
	}
}
