...
public class EditGroupTagDialog extends DialogContent {
	...
	public EditGroupTagDialog(MaterialExtentsWindow window) {
		super(window);
	}
	...
	protected void onLoad() {
	       ...
	       addDefaultButtons();
			...
			MaterialButton selectButton = new MaterialButton("수정");
	...
	private void checkDuplicateTagName(MaterialTextBox materialTextBox) {
		...
		if (checkTagname(targetSearchText)) {
			...
			commentLabel.setText("[" + inputBox.getText() + "] 사용 가능합니다. 수정 버튼을 선택해서 작업을 완료해 주세요.");
		}else{
			checkBox.setValue(false);
			commentLabel.setText("입력한 정보로는 태그를 수정할 수 없습니다.");
		}
	}
