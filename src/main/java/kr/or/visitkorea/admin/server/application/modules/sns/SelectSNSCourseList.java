 ...
 *
 */ 
@BusinessMapping(id="SELECT_SNSCOURSE_LIST")
public class SelectSNSCourseList extends AbstractModule{
...
//		System.out.println("SparameterObject.getString(\"des\") :: " + parameterObject.get("des"));
		...
		if (parameterObject.has("isOpen"))
			paramterMap.put("isOpen", parameterObject.getString("isOpen"));
		
		paramterMap.put("offset", parameterObject.getInt("offset"));
			
		List<HashMap<String, Object>> returnMap = sqlSession.selectList( 
			"kr.or.visitkorea.system.SnsMapper.selectSnsCourseList" , 
			paramterMap );
		List<HashMap<String, Object>> returnCnt = sqlSession.selectList( 
				"kr.or.visitkorea.system.SnsMapper.selectSnsCourseListCnt" , 
				paramterMap );
		
		JSONArray json = new JSONArray(returnMap);
		String convertJSONString = json.toString();

		System.out.println("SelectSnsCourseList.result.JSONArray :: " + convertJSONString);
