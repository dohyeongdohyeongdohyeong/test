...
@BusinessMapping(id="MONITORING_USERS")
public class MonitoringUsers extends AbstractModule {
	...
	public void run(JSONObject resultHeaderObject, JSONObject resultBodyObject) {
		...
		try {
			...
			JSONObject UserCollection = new JSONObject(response_string);
			if(UserCollection.has("user")) {
				
			if(UserCollection.get("user") instanceof JSONArray) {
				JSONArray Users = UserCollection.getJSONArray("user");
				resultBodyObject.put("Users", Users);
			} else if(UserCollection.get("user") instanceof JSONObject) {
				JSONObject Users = UserCollection.getJSONObject("user");
				resultBodyObject.put("Users", Users);
			} else {
				resultBodyObject.put("result", "Fail");
			}
			} else {
				resultBodyObject.put("result", "UserZero");
			}
